...
let tab= strArr.map(function (x) { return x.replace(/[/]/g,"").trim().toLowerCase();})
return tab ; 
}
...
function validate(num) {
    if ( (typeof num === "number") && (num <= 100) && (num% 2 == 0)) { return true}
    else return false ; 
}
...
function remove(arr, index) {
   ...
   let tab = arr.slice(0,index).concat(arr.slice(index+1 ,arr.length))
  return tab; // complete this statement
}
...
function formatPercentage(arr) {
    let tab= arr.map(function (x) { if (x>100){ x=100}
